...
public class AnalyzerActivity extends Activity
    ...
    Soundpays soundpays;
    TextView Text_scan_code;

    CalibrationLoad calibLoad = new CalibrationLoad();
    Button Btn_list_stop;

    // data for calibration of spectrum
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        soundpays = Soundpays.getInstance(AnalyzerActivity.this.getApplication());

        Btn_list_stop = (Button) findViewById(R.id.button);
        Text_scan_code = (TextView)findViewById(R.id.textView2);
        ...
        Btn_list_stop.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {

                //boolean pause = value.equals("Listen");
                String value = Btn_list_stop.getText().toString();
                if (value.contentEquals("Listen")) {
                    Btn_list_stop.setText("Stop");
                    if (samplingThread != null && samplingThread.getPause() != false) {
                        samplingThread.setPause(false);
                    }
                    analyzerViews.graphView.spectrogramPlot.setPause(false);
                    //beginAudioScan();
                } else if (value.contentEquals("Stop")) {
                    Btn_list_stop.setText("Listen");
                    if (samplingThread != null && samplingThread.getPause() != true) {
    ...
    private void receivedCode(final String code) {
        ...
        */
        // code="";
        AnalyzerActivity.this.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                stopAudioScan();

                if (code == "-1") {
                    Toast.makeText(AnalyzerActivity.this, "Mic Permission Needed", Toast.LENGTH_LONG).show();
